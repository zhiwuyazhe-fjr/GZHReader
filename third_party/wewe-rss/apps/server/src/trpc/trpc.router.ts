  ...
  accountRouter = this.trpcService.router({
    list: this.trpcService.protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const accountItems = await this.trpcService.listAccountsWithHealth();
        const startIndex = cursor
          ? accountItems.findIndex((item) => item.id === cursor) + 1
          : 0;
        const slicedItems = accountItems.slice(startIndex, startIndex + limit + 1);
        let nextCursor: typeof cursor | undefined = undefined;
        if (slicedItems.length > limit) {
          const nextItem = slicedItems.pop()!;
    ...
    add: this.trpcService.protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await this.trpcService.removeBlockedAccount(id);
